...
export const notificationHandlerModule = async function (snapshot, context) {
  ...
  const ownerDoc = admin
    .firestore()
    .doc("insta_users/" + context.params.userId);
...
function sendNotification(androidNotificationToken: string, activityItem) {
  let title: string;
  let body: string = "";
